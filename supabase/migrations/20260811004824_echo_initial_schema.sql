...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'bucket_preferences'
      and column_name = 'builtins'
  ) then
    update public.bucket_preferences as preferences
    set customs = preferences.customs || coalesce(
      (
        select jsonb_agg(legacy.value)
        from jsonb_each(preferences.builtins) as legacy(key, value)
        where jsonb_typeof(legacy.value) = 'object'
          and nullif(btrim(legacy.value ->> 'name'), '') is not null
          and not exists (
            select 1
            from jsonb_array_elements(preferences.customs) as custom(value)
            where lower(btrim(custom.value ->> 'name')) =
              lower(btrim(legacy.value ->> 'name'))
          )
      ),
      '[]'::jsonb
    );
...
revoke all on table
  ...
  public.bucket_preferences,
  public.standing_messages,
  public.sync_devices
